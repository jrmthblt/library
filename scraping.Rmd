...
First, we install the dependencies we need (the project is managed by Packrat).
...
Then, we try a simple scraping of one book to make sure it is working properly :
```{r one book scraping}
...
# for (i in 0:15) {
#   ftSummaries <- sapply(retrieveGenreCurrentPageBooksURLs(rootUrl, i, literaryGenres[1,2]), function(bookURL) {
...
# for (i in 0:12) {
#   histoSummaries <- sapply(retrieveAnyGenreCurrentPageBooksURLs("http://www.livredepoche.com/collection-litterature?sort_by=field_book_publication_date&sort_order=DESC&f[0]=field_tags%3A5721&f[1]=field_tags%3A76&page=", i), function(bookURL) {
#     book <- read_html(paste(rootUrl, bookURL, sep=""), encoding = "UTF-8")
#     Sys.sleep(scraperDelay)
#     return(retrieveBookSummary(book))
#   })
#   bookSummaries <- c(bookSummaries, histoSummaries)
# }


# Scraping "Littérature" books (20 are also included in imaginaire)
for (i in 0:127) {
  litSummaries <- sapply(retrieveAnyGenreCurrentPageBooksURLs("http://www.livredepoche.com/collection-litterature?sort_by=field_book_publication_date&sort_order=DESC&f[0]=field_tags%3A5721&f[1]=field_tags%3A31&page=", i), function(bookURL) {
    book <- read_html(paste(rootUrl, bookURL, sep=""), encoding = "UTF-8")
    Sys.sleep(scraperDelay)
    return(retrieveBookSummary(book))
  })
  bookSummaries <- c(bookSummaries, litSummaries)
}


dfBookSummaries <- data_frame(line = 1:length(bookSummaries), text = unlist(bookSummaries),genre=c(rep("Littérature",1279)))
...
write.csv(dfBookSummaries, file="/Users/jeromethibault/Documents/R/library/bookSummariesHisto.csv")
```