...
stopwords_fr2 = c('a','h','lundi','mardi','mercredi','jeudi','vendredi','samedi','dimanche',
                 ...
                 'l','c','resume','science-fiction','ou','tome','serie','roman','romans','collection',
                 'livre','gerard','dirigee','prix','king','auteur','stephen','histoire','etait','meme',
                 'recueil','hugo','quinze'
                 )

docs <- tm_map(docs, removeWords, stopwords_fr2)
...
La liste des mots les plus/moins fréquents dans le corpus peut être donnée selon l'approche suivante : 
```{r top/bottom}
...
dtm
dtm_m <- as.matrix(dtm) # renvoie pour chaque document sa fréquence tf-idf : cesar sur doc 1 = 0.2086247 ; alors = 0.03171596
dtm_m[1,1:20]
...
library(caret)
# TODO : il faut tenter caret en near zero variance. Faut-il le faire sur la dtm Tf ou TF-IDF ?
dtmTf_m <- as.matrix(dtmTf)
dim(dtmTf_m)
# nzv <- nearZeroVar(dtmTf_m, saveMetrics= TRUE)
# nzv$nzv
# nzv[nzv$nzv,][1:100,]
nzv <- nearZeroVar(dtmTf_m)
dtmTf_m_filtered <- dtmTf_m[, -nzv]
dim(dtmTf_m_filtered) # Caret a conservé 55 variables sur les 39584 ! Et on voit qu'il a conservé beaucoup de stop words... il faut refaire une passe complète.
...
dtmTf_m_filtered[68,] # fait manuellement depuis l'avertissement : empty document(s): 68 1210 2570 3923 4926
...
dtm_norm <- norm_eucl(as.matrix(dtm_m))
