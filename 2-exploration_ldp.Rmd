...
```{r termes fréquents}
# FIXME : ne devrait-on pas nettoyer uniquement le jeu d'entrainement et pas celui de test ?
# FIXME 2 : on veut prédire la collection d'appartenance pour un nouveau résumé saisi par l'utilisateur
...
Après suppression des stopwords, on remarque des mots proches les uns des autres : homme/hommes, femme/femmes, ... On va raciniser le texte pour résoudre ce problème :
...
Les résultats ne sont pas probants : homm renvoie hommage et homme par exemple.
...
# TODO : bounds ?
inspect(dtmTf)
as.matrix(dtmTf[1,1:20]) # on retrouve bien le nombre d'occurrences de chaque terme. Bizarre que ca s'appelle TF. Matrice ultra-sparse. sur le doc 1, cesar = 2 ; alors = 1. Bien le nombre d'occurrences

minfreq <- findFreqTerms(dtmTf, 1) # On fonctionne ici toujours sur la DTM pondéré par TF !!!
dtmTfIdf <- DocumentTermMatrix(docs, control=list(dictionary = minfreq, weighting=weightTfIdf))
dtmTfIdf
dtmTfIdf_m <- as.matrix(dtmTfIdf) # renvoie pour chaque document sa fréquence tf-idf : cesar sur doc 1 = 0.2086247 ; alors = 0.03171596
dtmTfIdf_m[1,1:20]
...
Autre méthode, en identifiant puis supprimant les variables ayant très peu de variance :
...
dtmTf_m_filtered <- dtmTf_m[, -nzv]
dim(dtmTf_m_filtered)


dim(dtmTfIdf_m)
nzvTfIdf <- nearZeroVar(dtmTfIdf_m)
dtmTfIdf_m_filtered <- dtmTfIdf_m[, -nzv]
dim(dtmTfIdf_m_filtered)

colnames(dtmTf_m_filtered)
```

Cette fonction conserve seulement 53 variables sur les 39k, que ce soit sur la matrice TF ou TF-IDF.


Dernière méthode pour réduire la dimension : on retire les mots les plus rares ie. ceux qui apparaissent dans moins d'1% des documents. Cela semble beaucoup plus efficace qu'en conservant les mots assez fréquents :

```{r réduction de la dimension par suppression des termes sparse}
dtmTfIdf <- removeSparseTerms(dtm, 0.99)
dtmTfIdf
...
On conserve ici 941 variables.


Enfin on construit notre DTM finale :

```{r construction finale de la DTM}
# Soit on s'appuie sur la matrice nettoyée avec removeSparseTerms :
finalDTM <- dtmTfIdf # On sauvegarde notre DTM pour la classification supervisée.
finalDTM_m <- dtmTfIdf_m
...
# Ou via caret ?
# library(FactoMineR)
# PCAResult <- PCA(finalDTM_m, scale.unit = FALSE, ncp = 10)
```
...
for (i in 1:length(collections)) {
  m <- finalDTM_m[dfBookSummaries$collection == collections[i],]
  wordsFreq <- sort(colSums(m),decreasing = TRUE)
  wordsFreq <- data.frame(word = names(wordsFreq), Freq = as.vector(wordsFreq)) # mise au format pour wordcloud2
  print(wordcloud2(data = wordsFreq[1:500,],minSize = 1, size = 3))
  }
...
TODO : A réécrire : on voit quelques mots typiques du genre Thriller comme "menace", le Fantastique est sur-représenté par son auteur emblématique "King" et la Littérature aborde à peu très tous les thèmes (amour, argent, voyage, ...). Bref c'est encore assez confus.


## TODO : Classification non supervisée LDA (Topic Modeling) / KMeans / SKmeans / CAH / ACP / ...

### Kmeans
```{r kmeans}
#http://michael.hahsler.net/SMU/CSE7337/ et http://michael.hahsler.net/SMU/CSE7337/install/tm.R

rowTotals <- apply(finalDTM_m , 1, sum) # calcule la somme des termes dans chaque document
finalDTM_m   <- finalDTM_m[rowTotals> 0, ] # retire les documents vides, il peut arriver après nettoyage de se retrouver avec des documents vides, par exemple si on travaille sur des documents très courts type tweet
dim(finalDTM_m)

### Normalize the vectors so Euclidean makes sense with kmeans
norm_eucl <- function(m) m/apply(m, MARGIN=1, FUN=function(x) sum(x^2)^.5)
dtm_norm <- norm_eucl(as.matrix(finalDTM_m))
...
# k = 2 to k.max (kmeans)
k.max <- 50
swss <- (nrow(dtm_norm)-1)*sum(apply(dtm_norm,2,var))
for(i in 2:k.max){
  set.seed(123)
  swss[i] <- sum(kmeans(dtm_norm, centers = i, iter.max = 20)$withinss)
}

# Plot the elbow (kmeans)
plot(1:k.max, swss, type = "b", pch = 19, 
     frame = FALSE, xlab = "Number of clusters k")
#abline(v = which.min(swss), lty = 2)


# cluster into x (optimal) clusters - kmeans
set.seed(123)
cl2 <- kmeans(dtm_norm, centers = 8, iter.max = 20) # Remplacer 'centers' par nombre optimal trouvé
table(cl2$cluster)
# show clusters using the first 2 principal components
plot(prcomp(dtm_norm)$x, col=cl2$cluster)
...
dim(finalDTM_m)
sk <- skmeans(x=finalDTM_m, 6)
## On regarde la r?partition dans les clusters
table(sk$cluster)/sum(table(sk$cluster))

for (i in 1:6){
  m <- finalDTM_m[sk$cluster==i,]
  wordsFreq <- sort(colSums(m),decreasing = TRUE)
  wordsFreq <- data.frame(word = names(wordsFreq), Freq = as.vector(wordsFreq)) # mise au format pour wordcloud2
  print(wordcloud2(data = wordsFreq[1:500,],minSize = 1, size = 3))}
```
